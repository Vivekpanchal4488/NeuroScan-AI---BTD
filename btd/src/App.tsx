...
function App() {
  ...
  useEffect(() => {
    const unlisten = addListener(({ user }) => {
      if (!user) { sessionStorage.removeItem('neuroscan-page'); setShowHome(true); setIsDoctor(false); resetAnalysis(); }
    });
    return () => unlisten();
  }, []);

  useEffect(() => {
    if (isSignedIn === false) { sessionStorage.removeItem('neuroscan-page'); setShowHome(true); setIsDoctor(false); resetAnalysis(); }
  }, [isSignedIn]);

  useEffect(() => {
    const keepAwake = setInterval(() => { fetch('https://brain-tumor-api-m3x7.onrender.com/').catch(() => {}); }, 600000);
  ...
  if (showHome) {
    return (
      <HomePage
        onGuestAccess={() => { sessionStorage.setItem('neuroscan-page', 'scan'); setShowHome(false); setIsDoctor(false); }}
        onDoctorAccess={() => { sessionStorage.setItem('neuroscan-page', 'doctor'); setShowHome(false); setIsDoctor(true); }}
      />
    );
  }
  ...
  if (isDoctor) {
    return (
      <DoctorScanPage onHome={() => { sessionStorage.removeItem('neuroscan-page'); setShowHome(true); setIsDoctor(false); resetAnalysis(); }} />
    );
  }
      ...
      <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Manrope', sans-serif", background: '#f9f9f5' }}>
        ...
        <header className="flex items-center justify-between px-4 sm:px-8 py-3 sm:py-4 border-b sticky top-0 z-50" style={{ background: '#f9f9f5', borderColor: '#e0e0d8' }}>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button
              onClick={() => { sessionStorage.removeItem('neuroscan-page'); setShowHome(true); resetAnalysis(); }}
              className="inline-flex items-center gap-1.5 sm:gap-2 text-sm font-semibold px-3 sm:px-4 py-2 rounded-lg transition-all duration-200"
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {!uploadedFile && (
                <div className="rounded-2xl border p-5 sm:p-8" style={{ background: '#fff', borderColor: '#e0e0d8', boxShadow: '0 4px 24px rgba(0,0,0,0.05)' }}>
                  ...
                  <h3 className="text-lg sm:text-xl font-bold text-center mb-5 sm:mb-6" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#0d0d0d' }}>
                    How it works
                  </h3>
                  <div className="space-y-4 sm:space-y-5">
                    {[
                      ...
                      <div key={step.n} className="flex items-start gap-3 sm:gap-4">
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold" style={{ background: '#e8eeff', color: '#3b5bdb' }}>
                          {step.n}
                        </div>
