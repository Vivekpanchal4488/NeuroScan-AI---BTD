...
interface DoctorScanPageProps { onHome: () => void; }

const inputStyle: React.CSSProperties = {
  border: '1.5px solid #e0e0d8', background: '#f9f9f5', color: '#0d0d0d',
  fontFamily: "'Manrope', sans-serif", width: '100%', padding: '10px 14px',
...
const DoctorScanPage: React.FC<DoctorScanPageProps> = ({ onHome }) => {
  ...
  const [processingState, setProcessingState] = useState<ProcessingState>({ isProcessing: false, progress: 0, stage: '' });
  ...
  useEffect(() => {
    if (user?.id) getDoctorPatients(user.id).then(setSavedPatients);
  }, [user]);

  const resetAnalysis = () => {
    setUploadedFile(null); setDetectionResult(null);
    setProcessingState({ isProcessing: false, progress: 0, stage: '' });
    setPatientName(''); setAge(''); setGender(''); setPhone(''); setNotes('');
    setFormSubmitted(false); setLastSavedId(null); setNotesSaved(false);
  };

  const handleFileUpload = async (file: UploadedFile) => {
    if (!file) { setUploadedFile(null); setDetectionResult(null); setProcessingState({ isProcessing: false, progress: 0, stage: '' }); return; }
    ...
    try {
      ...
      if (user?.id && patientName) {
        ...
        const savedId = await savePatientRecord({
          doctorId: user.id, patientName, age, gender, phone,
          scanDate: new Date().toLocaleString(),
          tumorDetected: result.hasTumor,
          tumorType: (result as any).tumorType,
          confidence: result.confidence, notes,
        });
  ...
  const handleDownload = () => {
    if (uploadedFile && detectionResult) downloadReport('brain-scan-report', detectionResult, uploadedFile);
  };

  if (showRecords) return <PatientRecords onBack={() => setShowRecords(false)} />;
    ...
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Manrope', sans-serif", background: '#f9f9f5' }}>
      ...
      <header className="flex items-center justify-between px-4 sm:px-8 py-3 sm:py-4 border-b sticky top-0 z-50" style={{ background: '#f9f9f5', borderColor: '#e0e0d8' }}>
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center" style={{ background: '#0d0d0d' }}>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {user && (
            <div className="text-xs sm:text-sm font-medium hidden md:block" style={{ color: '#555' }}>
              Dr. <span style={{ color: '#0d0d0d', fontWeight: 700 }}>{user.firstName || user.username}</span>
            </div>
          )}
          <div className="flex items-center gap-1.5 text-xs" style={{ color: '#2d9c2d' }}>
            <span className="w-2 h-2 rounded-full animate-pulse inline-block" style={{ background: '#2d9c2d' }} />
            <span className="hidden sm:inline">System Online</span>
          </div>
          <button
            onClick={async () => { await signOut(); sessionStorage.removeItem('neuroscan-page'); onHome(); }}
            className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-semibold px-2.5 sm:px-4 py-2 rounded-lg transition-all duration-200"
      ...
      {/* MAIN */}
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-10 py-6 sm:py-10">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <h2 className="font-extrabold mb-2" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 'clamp(28px, 2vw, 42px)', letterSpacing: '-0.02em', color: '#0d0d0d' }}>
            ...
            <span className="relative inline-block">
              ...
              <span className="absolute left-0 bottom-1 w-full rounded-sm" style={{ background: '#c8f04e', height: '8px', display: 'block', zIndex: -1 }} />
            </span>
          </h2>
          <p className="text-sm sm:text-base" style={{ color: '#666', fontWeight: 300 }}>
        ...
        {/* PATIENT FORM */}
        {!formSubmitted ? (
          <div className="rounded-2xl border p-5 sm:p-7 mb-6 sm:mb-8" style={{ background: '#fff', borderColor: '#e0e0d8', boxShadow: '0 4px 24px rgba(0,0,0,0.05)' }}>
            <h3 className="text-sm sm:text-base font-bold mb-4 sm:mb-5 flex items-center gap-2" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#0d0d0d' }}>
              <ClipboardList className="h-5 w-5" style={{ color: '#3b5bdb' }} /> Patient Details
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mb-4 sm:mb-5">
              {[
                { label: 'Patient Name', req: true, val: patientName, set: setPatientName, type: 'text', ph: 'Enter full name' },
                { label: 'Age', req: true, val: age, set: setAge, type: 'number', ph: 'Age in years' },
                { label: 'Phone Number', req: false, val: phone, set: setPhone, type: 'tel', ph: 'Contact number' },
              ].map(f => (
                <div key={f.label}>
            ...
            <div className="mb-4 sm:mb-5">
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)}
                placeholder="Any observations, symptoms or notes before scanning..." rows={3}
                style={{ ...inputStyle, resize: 'none' }}
                onFocus={e => (e.currentTarget.style.borderColor = '#0d0d0d')}
                onBlur={e => (e.currentTarget.style.borderColor = '#e0e0d8')} />
            </div>
            <button
              onClick={() => { if (!patientName.trim() || !age.trim()) { alert('Please enter patient name and age.'); return; } setFormSubmitted(true); }}
              className="w-full sm:w-auto px-6 sm:px-7 py-3 rounded-xl text-sm font-bold transition-all duration-200"
          ...
          <div className="rounded-2xl border p-3 sm:p-4 mb-6 sm:mb-8 flex flex-wrap items-center justify-between gap-3" style={{ background: '#f0fdf4', borderColor: '#bbf7d0' }}>
            <div className="flex items-center gap-2 flex-wrap text-xs sm:text-sm" style={{ color: '#166534' }}>
            ...
            <button onClick={() => setFormSubmitted(false)} className="text-xs px-3 py-1.5 rounded-lg font-semibold" style={{ background: '#dcfce7', color: '#166534', border: '1px solid #bbf7d0' }}>
              ✏️ Edit
            </button>
          </div>
        )}
        ...
        {saving && (
          <div className="mb-4 text-xs sm:text-sm font-medium flex items-center gap-2" style={{ color: '#3b5bdb' }}>
            <span className="animate-spin">⏳</span> Saving patient record...
          </div>
        )}
        {lastSavedId && !saving && (
          <div className="mb-4 text-xs sm:text-sm font-medium" style={{ color: '#16a34a' }}>
        ...
        {formSubmitted && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {detectionResult && (
                <div className="rounded-2xl border p-5 sm:p-6" style={{ background: '#fff', borderColor: '#e0e0d8', boxShadow: '0 4px 24px rgba(0,0,0,0.05)' }}>
                  <h3 className="text-sm sm:text-base font-bold mb-3" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#0d0d0d' }}>📝 Post-Scan Notes</h3>
                  <textarea value={notes} onChange={e => { setNotes(e.target.value); setNotesSaved(false); }}
                    placeholder="Add clinical observations or recommendations..." rows={4}
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    ...
                    {notesSaved && <span className="text-xs sm:text-sm font-medium" style={{ color: '#16a34a' }}>✅ Notes saved!</span>}
                  </div>
                </div>
              )}
              {!uploadedFile && (
                <div className="rounded-2xl border p-5 sm:p-8" style={{ background: '#fff', borderColor: '#e0e0d8', boxShadow: '0 4px 24px rgba(0,0,0,0.05)' }}>
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mx-auto mb-5 sm:mb-6" style={{ background: '#e8eeff' }}>
                    <svg className="h-7 w-7 sm:h-8 sm:w-8" style={{ color: '#3b5bdb' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                  </div>
                  <h3 className="text-lg sm:text-xl font-bold text-center mb-5" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#0d0d0d' }}>How it works</h3>
                  <div className="space-y-4">
                    {[
                      ...
                      <div key={step.n} className="flex items-start gap-3 sm:gap-4">
                        <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold" style={{ background: '#e8eeff', color: '#3b5bdb' }}>{step.n}</div>
                        <p className="text-sm pt-0.5 leading-relaxed" style={{ color: '#555' }}>{step.text}</p>
                      </div>
                    ))}
                  </div>
                  <div className="mt-6 sm:mt-8 rounded-xl p-4 sm:p-5 grid grid-cols-3 gap-2 text-center" style={{ background: '#0d0d0d' }}>
                    {[{ value: '88%', label: 'Accuracy' }, { value: '4', label: 'Tumor Classes' }, { value: '5600+', label: 'Images' }].map(s => (
                      <div key={s.label}>
                        <div className="font-extrabold text-lg sm:text-xl" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#c8f04e' }}>{s.value}</div>
        ...
        {detectionResult && (
          <div className="mt-6 sm:mt-10 text-center">
            <button onClick={resetAnalysis}
              className="inline-flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg text-sm font-semibold transition-all duration-200"
              style={{ background: '#0d0d0d', color: '#f9f9f5', border: '2px solid #0d0d0d' }}
              onMouseEnter={e => { const el = e.currentTarget; el.style.background = '#c8f04e'; el.style.color = '#0d0d0d'; el.style.borderColor = '#c8f04e'; }}
              onMouseLeave={e => { const el = e.currentTarget; el.style.background = '#0d0d0d'; el.style.color = '#f9f9f5'; el.style.borderColor = '#0d0d0d'; }}>
        ...
        {savedPatients.length > 0 && (
          <div className="mt-8 sm:mt-12">
            <button onClick={() => setShowPatients(!showPatients)}
              className="flex items-center gap-2 text-sm font-bold px-4 sm:px-5 py-2.5 rounded-xl mb-4 transition-all duration-200"
              style={{ background: '#e8eeff', color: '#3b5bdb', border: '1.5px solid #c7d7ff' }}>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm" style={{ minWidth: '600px' }}>
                    <thead style={{ background: '#0d0d0d' }}>
                      <tr>
                        {['Patient Name', 'Age', 'Gender', 'Scan Date', 'Result', 'Confidence', 'Notes'].map(h => (
                          <th key={h} className="px-3 sm:px-4 py-3 text-left text-xs font-semibold whitespace-nowrap" style={{ color: 'rgba(255,255,255,0.6)' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {savedPatients.map((p, i) => (
                        <tr key={i} style={{ borderTop: '1px solid #e0e0d8', background: i % 2 === 0 ? '#ffffff' : '#f9f9f5' }}>
                          <td className="px-3 sm:px-4 py-3 font-semibold text-xs sm:text-sm" style={{ color: '#0d0d0d' }}>{p.patientName}</td>
                          <td className="px-3 sm:px-4 py-3 text-xs sm:text-sm" style={{ color: '#555' }}>{p.age}</td>
                          <td className="px-3 sm:px-4 py-3 text-xs sm:text-sm" style={{ color: '#555' }}>{p.gender || '—'}</td>
                          <td className="px-3 sm:px-4 py-3 text-xs whitespace-nowrap" style={{ color: '#888' }}>{p.scanDate}</td>
                          <td className="px-3 sm:px-4 py-3">
                            <span className="px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap" style={{ background: p.tumorDetected ? '#fee2e2' : '#dcfce7', color: p.tumorDetected ? '#dc2626' : '#16a34a' }}>
                              {p.tumorDetected ? `⚠ ${p.tumorType || 'Tumor'}` : '✓ No Tumor'}
                            </span>
                          </td>
                          <td className="px-3 sm:px-4 py-3 text-xs sm:text-sm" style={{ color: '#555' }}>
                            {p.confidence ? `${Number(p.confidence).toFixed(1)}%` : '—'}
                          </td>
                          <td className="px-3 sm:px-4 py-3 text-xs max-w-xs truncate" style={{ color: '#888' }}>{p.notes || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
